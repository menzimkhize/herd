# USAGE
# python elephants.py --mode train
# python elephants.py --mode investigate
# python elephants.py --mode predict --image examples/elephants_01.jpg
# python elephants.py --mode predict --image examples/elephants_01.jpg \
# 	--weights logs/elephants20181018T0624/mask_rcnn_elephants_0015.h5
...
DATASET_PATH = os.path.abspath("elephants")
...
class elephantsConfig(Config):
	...
	# number of classes (+1 for the backgelephant)
	NUM_CLASSES = len(CLASS_NAMES) + 1

class elephantsInferenceConfig(elephantsConfig):
...
class elephantsDataset(utils.Dataset):
	def __init__(self, imagePaths, annotPath, classNames, width=640):
	...
	def load_elephants(self, idxs):
		# loop over all class names and add each to the 'elephants'
		# dataset
		for (classID, label) in self.classNames.items():
			self.add_class("elephants", classID, label)
		...
		for i in idxs:
			...
			self.add_image("elephants", image_id=filename,
	...
	def load_mask(self, imageID):
		...
		for (i, region) in enumerate(annot["regions"]):
			...
			img = np.zeros((info["height"], info["width"], 3), dtype = "uint8")

			# grab the shape and region attributes
			sa = region["shape_attributes"]
			ra = region["region_attributes"]

			all_points_x = np.array(sa["all_points_x"])
			all_points_y = np.array(sa["all_points_y"])
			#print(regionMask.shape)
			mask_mat = np.array([all_points_x,all_points_y])
			#print(a.shape)

			#print(a)

			mask_mat_t = np.transpose(mask_mat)
			#print(b.shape)

			#print(b)
			#print(all_points_x.shape)
			#print(all_points_y.shape)
			...
			#print(ratio)
			mask_mat_r = np.array([(mask_mat_t * ratio).astype(np.int32)])
			...
			#cv2.polylines(img, b, True, (0,255,255), 1)
			cv2.fillPoly(img, pts = mask_mat_r , color=(255,255,255))
			#cv2.polylines(regionMask, b, True, (0,255,255),3)
			#print(points.shape)
			#print(b.shape)
			#print(img.shape)

			#cv2.imshow("Elephant Herd", img)
			#cv2.waitKey(0)

			#cv2.circle(regionMask, (cX, cY), r, 1, -1)
			masks[:, :, i] = img[:,:,1]
			#print(np.ones((masks.shape[-1]))
...
if __name__ == "__main__":
	...
	if args["mode"] == "train":
		...
		trainDataset = elephantsDataset(IMAGE_PATHS, ANNOT_PATH,
			CLASS_NAMES)
		trainDataset.load_elephants(trainIdxs)
		trainDataset.prepare()

		# load the validation dataset
		valDataset = elephantsDataset(IMAGE_PATHS, ANNOT_PATH,
			CLASS_NAMES)
		valDataset.load_elephants(valIdxs)
		valDataset.prepare()

		# initialize the training configuration
		config = elephantsConfig()
		config.display()
	...
	# check to see if we are predicting using a trained Mask R-CNN
		...
		config = elephantsInferenceConfig()
